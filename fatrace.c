             ...
             const struct timeval *event_time)
{
    ...
    if (proc_fd >= 0) {
        ...
        if (option_ancestors) {
            ...
            if (len >= 0 && sscanf(statbuf, "%*d (%*[^)]) %*c %d", &ppid) == 1)
                ...
                add_problem("Could not read /proc/PID/stat, cannot determine any ancestors.");
        }
        ...
        if (option_exe) {
            ...
            if (len >= 0) {
                ...
                add_problem("Failed to readlink /proc/PID/exe, cannot determine executable path.");
            }
        }
    ...
    if (!got_procname) {
        if (data->pid == procname_pid) {
            ...
            add_problem("However, cached comm 2 is usable.");
    ...
    if (option_json) {
        ...
        if (option_exe == 1 && got_exepath) {
            ...
            if (print_json_str("exe", exepath))
                add_problem("exe contains invalid UTF-8, exe_raw contains the bytes.");
        }
    } else {
            ...
            if (ppid_dir_fd >= 0) {
                ...
                if (len >= 0) {
                    ...
                    if (option_json) {
                        ...
                        if (print_json_str("comm", p_procname)) {
                            if (!problem_p_comm_raw)
                                add_problem("In an ancestor, comm contains invalid UTF-8, comm_raw contains the bytes.");
                    ...
                    if (!problem_p_comm_not_found)
                        add_problem("In an ancestor, failed to read /proc/PPID/comm, cannot determine comm.");
                ...
                if (option_exe) {
                    ...
                    if (len >= 0) {
                        ...
                        if (option_json) {
                            ...
                            if (print_json_str("exe", exepath)) {
                                if (!problem_exe_raw)
                                    add_problem("In an ancestor, exe contains invalid UTF-8, exe_raw contains the bytes.");
                        ...
                        if (!problem_exe_not_found)
                            add_problem("In an ancestor, failed to readlink /proc/PPID/exe, cannot determine executable path.");
                ...
                if (ppid == 1)
                    ...
                    if (len >= 0) {
                        if (sscanf(statbuf, "%*d (%*[^)]) %*c %d", &ppid) != 1) {
                            add_problem("In an ancestor, could not parse /proc/PPID/stat, ancestors list is incomplete.");
                            ppid = 0; // stop here
                        }
                    } else {
                        add_problem("Could not read all /proc/PPID/stat, ancestors list is incomplete.");
                    }
                }
                close (ppid_dir_fd);
            }
